...
class Corpus(object):
    ...
    def get_dataset(self, batch_size, num_steps, num_examples=None, shuffle=True):
        ...
        source_vocab = Vocab(source, min_freq=1, reserved_tokens=['<pad>', '<bos>', '<eos>'], savepath='data/fra-eng/source_vocab')
        target_vocab = Vocab(target, min_freq=1, reserved_tokens=['<pad>', '<bos>', '<eos>'], savepath='data/fra-eng/target_vocab')
